...
export const MapProvider = ({ children, apiKey }) => {
  ...
  const removeMarkers = () => {
    removeElementsByClass('houseshare-marker');
  };

  const removePopups = () => {
    removeElementsByClass('houseshare-popup');
  };
  ...
  const addMarker = (map, position) => {
    ...
    markerDiv.className = 'houseshare-marker';
