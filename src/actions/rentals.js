...
export const verifyRentalOwner = (rentalId) => {
  return houseshareAxios.get(`/rentals/${rentalId}/verify-user`);
};
...
export const fetchRentals = (location) => (dispatch) => {
  ...
  houseshareAxios.get(query).then((res) => {
...
export const fetchRentalById = (rentalId) => async (dispatch) => {
  ...
  const res = await houseshareAxios.get(`/rentals/${rentalId}`);
...
export const createRental = (rental) => {
  return houseshareAxios.post('/rentals', rental);
};
